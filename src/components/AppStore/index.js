...
class AppStore extends Component {
  ...
  changeTab = tabId => {
    this.setState({categoryApp: tabId})
  }

  searchInput = event => {
    const searchInput = event.target.value
    const searchInputLower = searchInput.toLowerCase()
    this.setState({onSearch: searchInputLower})
  }

  render() {
    const {categoryApp, onSearch} = this.state
    const filteredApps = appsList.filter(
      eachApp =>
        eachApp.category === categoryApp &&
        eachApp.appName.toLowerCase().includes(onSearch),
    )
    return (
      <div className="app-store-bcg-container">
        <h1> App Store </h1>
        <div className="search-container">
          <input
            ...
            className="search-input-app-name"
          />
          <img
        ...
        <ul className="app-categories-container">
          {tabsList.map(eachTab => (
            <TabItem
              eachTab={eachTab}
              key={eachTab.tabId}
              changeTab={this.changeTab}
              isCurrentTab={tabsList.tabId === categoryApp}
            />
          ))}
        </ul>
